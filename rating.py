...
# лесенке тиров, что видит пользователь.
#
# Два механизма против завышения на нетипичных снимках:
#
# 1. Замеры зажимаются в границы обучающей выборки. За её пределами линейная
#    модель экстраполирует вслепую: широкое лицо получало прибавку без
#    всякого потолка, потому что «чем шире, тем лучше» верно лишь в той
#    зоне, где примеры были.
#
# 2. Редкие широкие лица (face_aspect выше 0.90) обучаются с повышенным
#    весом. Их в разметке 35 против 569, и без этого модель их просто не
#    замечала — а именно на них приходились жалобы: лоб, закрытый шапкой или
#    чёлкой, занижает измеренную высоту лица, и все производные признаки
#    взлетают.
#
# Цена — два процентных пункта общей точности. Осознанный размен: систему
# судят по грубым промахам, а не по среднему.
...
MODEL_LO = [-2.464391, 0.192364, 0.885318, 0.465238, 1.247368, 0.738302, 126.315986, 0.33956, 0.26401, 0.740836, 0.833323, 0.738302, 0.171989, 0.244952, 0.404773, 0.33956, 0.117743, -0.215642, 0.620498, -0.073151, 0.507832, 0.003631]
MODEL_HI = [9.657048, 0.410834, 0.993642, 0.944434, 1.748577, 0.844155, 147.138375, 0.462283, 0.347604, 0.939767, 0.927163, 0.844155, 0.213013, 0.392348, 0.504103, 0.462283, 0.15975, -0.037994, 0.782265, 0.434668, 0.795848, 0.086038]
MODEL_MEAN = [4.307859, 0.310317, 0.96915, 0.722021, 1.488916, 0.795115, 138.610417, 0.399633, 0.300812, 0.833599, 0.880994, 0.795115, 0.192623, 0.319852, 0.455978, 0.399633, 0.134924, -0.119399, 0.702077, 0.227806, 0.641833, 0.041136, 2.23157, 0.63281, 0.776626, 19235.112739, 26.315089, 0.696697, 0.16056, 0.090858]
MODEL_SCALE = [2.785218, 0.043283, 0.024237, 0.11527, 0.121237, 0.024527, 4.718582, 0.029214, 0.019227, 0.04255, 0.021817, 0.024527, 0.009555, 0.032236, 0.021875, 0.029214, 0.008992, 0.042326, 0.038153, 0.115051, 0.073133, 0.0197, 0.361601, 0.038923, 0.038443, 1296.60379, 23.238765, 0.071211, 0.023383, 0.0117]
MODEL_COEF = [-0.073403, -0.404707, 0.151001, 1.753978, 0.261544, -0.46795, -2.143757, -0.373494, 0.906919, 1.182171, -0.940157, -0.46795, 0.257547, 0.216968, -0.587493, -0.373494, -0.08226, -2.045657, -0.210021, 2.26943, -0.076738, -1.050041, 0.166805, 3.286388, -0.758257, 1.989486, 0.54624, -2.144522, 2.372565, -1.532417]
MODEL_INTERCEPT = 4.751447
MODEL_VERSION = "2026-08-09b · 569 фото"
...
# уровень.
RAW_POINTS = [-0.3112, 2.0711, 2.3166, 2.6158, 3.0695, 3.3138, 3.5906, 3.9216, 4.1986, 4.4756, 4.6889, 4.9406, 5.271, 5.6008, 5.8042, 6.1497, 6.4128, 6.8263, 7.1521, 7.79, 10.0126]
...
def model_score(metrics: "FaceMetrics") -> float:
    ...
    # Зажимаем в границы обучающей выборки: дальше модель не видела ничего
    # и её линейная экстраполяция ничем не обоснована.
    clipped = {
        key: _clamp(getattr(metrics, key), lo, hi)
        for key, lo, hi in zip(MODEL_KEYS, MODEL_LO, MODEL_HI)
    }
    values = [clipped[key] for key in MODEL_KEYS]
    values += [clipped[key] ** 2 for key in MODEL_SQUARES]
