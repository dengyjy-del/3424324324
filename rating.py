...
# Модель обучена на ручной разметке владельца — 253 фотографии по той самой
# лесенке тиров, что видит пользователь. Проверка на отложенных данных
# (10-кратная кросс-валидация): корреляция 0.75, тир угадан в 42% случаев
# против 14% у случайного выбора из семи тиров.
#
# К линейным признакам добавлены квадраты части из них. Это позволяет
# модели видеть «слишком много» так же плохо, как «слишком мало»: например,
# и чрезмерно широкое, и чрезмерно узкое лицо получают штраф, тогда как
# чисто линейная зависимость наказывала бы только одну сторону.
#
# Оценки самих пользователей в обучение не идут: они завышают себе примерно
# на два балла, и добавление их даже с поправкой ухудшало результат —
# смещение у каждого своё, а не общее.
...
MODEL_MEAN = [4.102994, 0.31175, 0.965472, 0.716475, 1.471593, 0.793331, 138.157939, 0.400576, 0.303369, 0.831205, 0.880449, 0.793331, 0.191606, 0.319187, 0.455566, 0.400576, 0.134992, -0.128722, 0.706246, 0.202374, 0.634836, 0.045606, 2.183197, 0.63006, 0.775751, 19116.349417, 26.359199, 0.693135, 0.161588, 0.092553]
MODEL_SCALE = [3.086201, 0.053321, 0.029831, 0.129838, 0.132702, 0.026188, 5.360356, 0.033569, 0.02281, 0.047268, 0.023688, 0.026188, 0.01103, 0.036688, 0.025771, 0.033569, 0.010325, 0.046917, 0.042955, 0.132239, 0.077072, 0.021835, 0.396117, 0.041537, 0.041814, 1464.613128, 24.617108, 0.07922, 0.026959, 0.013853]
MODEL_COEF = [0.527055, -0.24059, 0.193209, 0.883187, 0.729524, 0.834157, 0.10962, 0.44618, 1.039401, 0.499836, -0.742223, 0.834157, 0.475413, 0.577818, -0.233303, 0.44618, 0.102716, -0.670129, -0.071676, 0.892616, -0.042457, -0.86667, -0.397745, 0.565633, -1.497274, -0.222784, -0.218621, -0.541498, -0.455188, -1.298232]
MODEL_INTERCEPT = 3.72253
MODEL_PIVOT = 3.7225
