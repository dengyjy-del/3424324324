...
# Модель обучена на ручной разметке владельца — 454 фотографии по той самой
# лесенке тиров, что видит пользователь. Проверка на отложенных данных
# (10-кратная кросс-валидация): корреляция 0.78.
#
# Калибровка подобрана не по максимуму попаданий в тир, а по тому, чтобы
# медиана каждого тира легла близко к разметке. Иначе выигрыш в процентах
# получается за счёт сжатия краёв: модель перестаёт ставить и очень низкие,
# и очень высокие баллы, а именно они больше всего заметны пользователю.
#
# Признаки кожи и бровей пока не входят: снимков с ними вдвое меньше, и на
# такой выборке потеря от нехватки примеров перевешивает пользу.
#
...
MODEL_MEAN = [4.455859, 0.310385, 0.967422, 0.7212, 1.490736, 0.795525, 138.546557, 0.399924, 0.300946, 0.834365, 0.881397, 0.795525, 0.192757, 0.319977, 0.455614, 0.399924, 0.135186, -0.119393, 0.701184, 0.227622, 0.644098, 0.041065, 2.237462, 0.633489, 0.777353, 19219.261291, 28.012842, 0.698088, 0.160883, 0.090993]
MODEL_SCALE = [2.856249, 0.04765, 0.027173, 0.120548, 0.12316, 0.025083, 4.910475, 0.030715, 0.020594, 0.043857, 0.022196, 0.025083, 0.0103, 0.033997, 0.02311, 0.030715, 0.00959, 0.044526, 0.040413, 0.12202, 0.075561, 0.020729, 0.368641, 0.039827, 0.039151, 1347.120326, 24.210828, 0.073683, 0.024681, 0.012486]
MODEL_COEF = [0.357372, -0.216718, 0.151917, 0.71616, 0.931389, 0.770915, 0.07209, 0.370554, 0.646591, 0.228695, -0.64775, 0.770915, 0.286895, 0.257968, -0.083179, 0.370554, -0.091298, -0.146442, 0.090164, 0.962346, 0.090346, -0.416849, -0.303507, 0.584058, -1.163428, 0.041086, -0.109014, -0.466144, -0.010505, -0.925177]
MODEL_INTERCEPT = 4.682379
MODEL_PIVOT = 4.6824
...
# уровень.
RAW_POINTS = [-0.4523, 1.8716, 2.4613, 2.7641, 3.1347, 3.3915, 3.5602, 3.8125, 4.1825, 4.5172, 4.7245, 4.9643, 5.1727, 5.456, 5.7232, 6.0022, 6.2846, 6.5457, 6.9523, 7.5601, 9.566]
TARGET_POINTS = [0.8, 1.3, 1.9, 1.9, 2.56, 2.8, 3.2, 3.2, 3.8, 4.1, 4.6, 5.115, 5.9, 6.1, 6.3, 6.9, 6.9, 7.0, 7.4, 8.0, 10.0]
