...
import { analyseImage, drawMesh, detectorFailed } from "/static/facemesh.v3.js";
import { renderCard, cardData, availableThemes, toBlob } from "/static/sharecard.v3.js";
...
async function loadSeedList() {
  ...
  try {
    const data = await api("/api/peer/seedlist");
    if (!data.items.length) {
      box.innerHTML =
        '<p class="tiny" style="color:var(--ink-3)">Анкет наполнения пока нет.</p>';
      return;
    }
      ...
      .map(
        ...
        <div class="seed-row${item.custom ? "" : " auto"}" data-key="${item.key}"
          ...
          <div class="seed-actions">
            ...
            <button data-act="del" class="danger" title="Удалить">✕</button>
          </div>
        </div>`
      )
      .join("");
...
async function saveSeed(row) {
  haptic();
  const body = new FormData();
  body.append("key", row.dataset.key);
  body.append("name", row.querySelector(".seed-name").value);
  body.append("age", row.querySelector(".seed-age").value || "0");

  try {
    await api("/api/peer/seedlist", { method: "POST", body });
    // Имя задано вручную — снимаем приглушение
...
async function deleteSeed(row) {
  haptic("medium");
  try {
    await api(`/api/peer/seedlist/${row.dataset.key}`, { method: "DELETE" });
    row.remove();
    toast("Анкета удалена");
    loadSeedBox();
    loadDeck();
...
function initPeer() {
  $("pr-seed-toggle").addEventListener("click", () => {
    ...
    $("pr-seed-toggle").textContent = opening
      ? "Скрыть анкеты"
      : "Управлять анкетами";
    if (opening) loadSeedList();
  });
...
async function boot() {
  ...
  try {
    ...
    // Только через safePaint: сбой отрисовки здесь прерывал весь запуск —
    // не показывались вкладки, не вызывался enterApp, и человек оставался
    // на экране онбординга с сообщением об ошибке.
    safePaint(() => paintStats(session.stats, session.user));
    ...
    toast(error.message);
    console.error("Запуск прерван", error);
  }
}
